...
#
# Usage:
#   sh utils/add-new-auth-keys.sh              # Interactive: prints keys, prompts to update .env
#   sh utils/add-new-auth-keys.sh --update-env # Prints keys and writes them to .env
#   sh utils/add-new-auth-keys.sh | tee keys   # Non-interactive: prints keys only
#
# Prerequisites:
#   - .env file with JWT_SECRET set (run generate-keys.sh first)
#   - node (>= 16) or docker
#
# Source: https://github.com/supabase/supabase/blob/master/docker/utils/add-new-auth-keys.sh
#
